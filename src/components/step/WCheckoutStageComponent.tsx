...
import { TipSelection, ITOTALS, ICREDIT_RESPONSE, OrderFulfillment, CartEntry, GenerateServiceTimeDisplayString } from '../common';
...
export function WCheckoutStage({ linearCart, fulfillment, totals, menu, creditResponse }: ICheckoutStage) {
  ...
  const submitWithSquare = () => {
    ...
    setOrderSubmissionResponse(null);
  }
  const [useCreditCheckbox, setUseCreditCheckbox] = useState(false);
  
  const serviceTimeDisplayString = useMemo(() => GenerateServiceTimeDisplayString(fulfillment), [fulfillment]);
    ...
    if (!orderSubmissionResponse) {
      return (
        <div>
          ...
          <WCheckoutCart menu={menu} linearCart={linearCart} fulfillment={fulfillment} totals={totals} creditResponse={creditResponse} />
